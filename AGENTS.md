...
- **Activation has three links and conflating them is the expensive mistake.** `/t/<templateId>` renders a template, creates nothing and is meant to spread; `/t/<templateId>/use` mints the visitor's own copy behind sign-in; `/claim/<code>` is single use and mints a paid buyer's. A code is a bearer token in a URL, the database is the only thing that hashes one, and a claim is idempotent because a second tap must open the invitation somebody already paid for and never a spent-code error. The copy link is the opposite: no code, so two presses are two copies. Every link is built from `NEXT_PUBLIC_SITE_URL` and no host is hardcoded anywhere. `docs/activation.md`.
- **The open copy link belongs to the free launch and MUST be replaced before the first paid listing publishes.** An open copy link plus a price is a free product; `ip-decision-order-verification` replaces it and is still open. It exists because the first template is free (2026-08-24, *"LET'S MAKE one link for all for now"*), and it is why the paid route stays untouched rather than tidied away. Signing in there creates an account, which is the second authorisation for becoming a customer and the other half to take back. `src/lib/activation/copy.ts`.
