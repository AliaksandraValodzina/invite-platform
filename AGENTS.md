...
**The product is Mirthly, and its primary domain is `mirthly.app`.** The repository, the Vercel project and the Supabase project are all still *named* `invite-platform`; renaming them is a separate action nobody has asked for. **The name is declared once, in the root layout**, as a title default plus a `%s - Mirthly` template, so no route repeats it; a live guest page opts out with `title: { absolute }` because the tab over an invitation belongs to the buyer, and `tests/unit/branding/name.test.ts` fails on the working title reappearing anywhere in `src/`. **How it is deployed is in `docs/hosting.md`** and the deployment shape is pinned in `vercel.json` rather than in a dashboard, for the same reason schema lives in migrations. Five things there are worth knowing before touching a hosted environment: the hosted database can be behind `main` and nothing says so out loud, the Supabase project's own Site URL and redirect allow-list are not schema and will silently break a paid activation if wrong, a deployment with no R2 variables has no object store at all, and **every Vercel environment variable on this project is stored `sensitive`, so no value can ever be read back** - `vercel pull` writes the literal `"[SENSITIVE]"` and anything in CI or in a build that treats that file as a source of values gets a placeholder, and **the address Mirthly sends from cannot receive a reply**: Supabase's SMTP settings have no `Reply-To` field and the auth service never writes that header, so a buyer's reply to a sign-in email is lost no matter what is configured. Mail the app sends itself is the opposite case, because Resend's API does take `reply_to`; the app sends none yet.
...
**Not in Phase 0:** payments, seller accounts, marketplace, freemium, custom domains, photo sharing. Filling a template in is built (`docs/editing.md`), and so is composing it (`docs/composition.md`): sections move, come off and go back, and the buyer picks their colours. A catalogue of designs to add a section from is not built and is gated on `ip-product-plan-decision-art-sourcing-and-capacity`. Activation is built (`docs/activation.md`): a claim link mints a paid buyer's copy, the free launch's open link at `/t/<templateId>/use` mints anybody's, and publish and unpublish work.
...
- **The template line is three themes, not one: Deckle & Deboss, Masthead, Foil & Midnight.** Their separateness is the product, so do not harmonise, rename or add a fourth. Every value in `templates/themes/` for those three is quoted from the design direction report that produced them and is not to be adjusted by eye. That report is not in this repository; `docs/design-directions.md` is, and it is the map from the report to this format, including what it needed that the format did not have.
...
- **RSVP data is guests' PII** — names, emails, dietary and health notes. The statement that says so ships at `/privacy` and `/terms`, and `tests/unit/legal/retention.test.ts` holds the days it prints to the days the sweep uses. Australian Privacy Act, GDPR if EU guests reply. Both are reachable from every page a stranger can arrive on through `src/components/site-footer.tsx`, which is drawn in the app's own type and always outside a `ThemeScope`, because that strip is the product talking and not the invitation. The point of collection carries its own link: the RSVP block names `/privacy` beside the fields.
...
- **Activation has four links and conflating them is the expensive mistake.** `/t/<templateId>` renders a template, creates nothing and is meant to spread; `/t/<templateId>/use` mints the visitor's own copy behind sign-in; `/claim/<code>` is single use and mints a paid buyer's; `/order` takes a typed Etsy order number. A code is a bearer token in a URL, the database is the only thing that hashes one, and a claim is idempotent because a second tap must open the invitation somebody already paid for and never a spent-code error. The copy link is the opposite: no code, so two presses are two copies. Every link is built from `NEXT_PUBLIC_SITE_URL` and no host is hardcoded anywhere. `docs/activation.md`.
- **The open copy link belongs to the free launch and MUST be withdrawn before the first paid listing publishes.** An open copy link plus a price is a free product. What replaces it is built, so this is a withdrawal and no longer a decision. It exists because the first template is free (2026-08-24, *"LET'S MAKE one link for all for now"*). Signing in there creates an account, which is the other half to take back. `src/lib/activation/copy.ts`.
- **A buyer proves a purchase by typing their Etsy order number at `/order`, checked against a list the captain loads in batches.** The captain's decision, taken twice, and the reason is the buyer being self-serve without Etsy API approval. `public.order_numbers` is its own table and not a row in `activation_codes`, because a code is a hundred bits this platform minted and an order number is ten guessable digits printed on a receipt: one column for both would make the codes table's own security argument untrue. Numbers are stored hashed with the last four in the clear, single use against one account, and guessing is capped in the database because it cannot be capped in a function's memory. `scripts/load-orders.ts` loads, `scripts/list-orders.ts` reconciles. `docs/orders.md`.
- **One published invitation at a time per account; drafts and copies are unlimited.** With an open copy link that is the only thing between one free template and somebody running a wedding business on it: a published event costs hosting for its whole term and a draft costs nothing. Enforced by `public.events_publish_limit` under an advisory lock, against `authenticated`, so no route skips it and no two tabs race it; the service role is outside it on purpose, which is what lets fixtures seed the four serving states. The editor's read of which invitation is in the way is the sentence, never the rule (`src/lib/editor/publish-limit.ts`).
...
**Never cite a path that is not in this repository.** Two entries here pointed at `data/...` files that exist only in the operator's own working directory, which is a dead reference for anybody who clones this. A brief may hand you such a path; the fact it carries belongs in this file, the path does not.
