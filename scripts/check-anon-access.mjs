...
function resolveConfig () {
  ...
  return { url: SUPABASE_URL.replace(/\/$/, ''), anonKey: SUPABASE_ANON_KEY, serviceKey: SUPABASE_SERVICE_ROLE_KEY }
}
...
async function seed () {
  const owner = await auth('admin/users', {
    ...
    body: { email: `owner-${stamp}@example.test`, password: `Owner-${stamp}-pw!`, email_confirm: true }
  })
  created.users.push(owner.id)

  const stranger = await auth('admin/users', {
    key: config.serviceKey,
    body: { email: `stranger-${stamp}@example.test`, password: `Stranger-${stamp}-pw!`, email_confirm: true }
  })
  ...
  return { owner, stranger, strangerToken: strangerSession.access_token, ownerToken: ownerSession.access_token }
}

async function cleanup () {
  if (created.event) await rest(`events?id=eq.${created.event.id}`, { ...svc, method: 'DELETE' })
  if (created.template) await rest(`templates?id=eq.${created.template.id}`, { ...svc, method: 'DELETE' })
...
async function main () {
  ...
  const seedCheck = await rest(`rsvps?select=guest_name,dietary_notes&id=eq.${created.rsvp.id}`, svc)
  check(
  ...
  const strangerAccounts = await rest('accounts?select=owner_id', { key: config.anonKey, token: strangerToken })
  check(
    'a signed-in stranger sees only their own account row',
    strangerAccounts.ok && strangerAccounts.json?.length === 1 && strangerAccounts.json[0].owner_id !== owner.id,
  ...
  const ownerReads = await rest(`rsvps?select=guest_name,dietary_notes`, { key: config.anonKey, token: ownerToken })
  check(
...
if (failures.length > 0) {
  console.log('\nFailures:')
  for (const failure of failures) console.log(`  - ${failure.description}${failure.detail ? `: ${failure.detail}` : ''}`)
  process.exit(1)
}
